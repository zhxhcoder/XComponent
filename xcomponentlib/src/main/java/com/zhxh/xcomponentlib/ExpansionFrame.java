...
public class ExpansionFrame extends FrameLayout {
    ...
    public ExpansionFrame(Context context, AttributeSet attrs) {
        super(context, attrs);

        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.ExpansionFrame);
            duration = a.getInt(R.styleable.ExpansionFrame_Xduration, DEFAULT_DURATION);
            expansion = a.getBoolean(R.styleable.ExpansionFrame_Xexpanded, false) ? 1 : 0;
            orientation = a.getInt(R.styleable.ExpansionFrame_android_orientation, VERTICAL);
            float parallax = a.getBoolean(R.styleable.ExpansionFrame_Xtranslate_children, true) ? 1 : 0;
            parallax = a.getFloat(R.styleable.ExpansionFrame_Xparallax, parallax);
            a.recycle();
